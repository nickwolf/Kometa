...
def make_imdb_search(post_response=None, post_side_effect=None):
    ...
    The module-level logger is None during tests, but the search path calls
    logger.ghost/exorcise, so patch it with a no-op mock via monkeypatch in the
    tests that need it.
    """
    imdb = IMDb(requests=MagicMock(), cache=None, default_dir="/tmp")
...
def _silence_logger(monkeypatch):
    """Provide a no-op logger so logger.ghost/exorcise calls don't crash tests."""
